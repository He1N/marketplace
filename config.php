...
class Database
{
     ...
     * 
     * @param int $user_id User ID
     * @param array $cart_items Cart items with product details and totals
     * @param array $shipping_info Shipping information
     * @param string $payment_method Payment method
     * @return int|bool Order ID on success, false on failure
     */
    public function createOrder($user_id, $cart_items, $shipping_info, $payment_method)
    {
        try {
            ...
            foreach ($cart_items as $item) {
                $total_amount += $item['price_current'] * $item['number_of_products'];
            }

            // Verificar stock antes de procesar la orden
            foreach ($cart_items as $item) {
                $product = $this->select('products', '*', "WHERE id = {$item['id']}")[0];
                if ($product['quantity'] < $item['number_of_products']) {
            ...
            foreach ($cart_items as $item) {
                ...
                $new_quantity = $product['quantity'] - $item['number_of_products'];
                $this->executeQuery("UPDATE products SET quantity = $new_quantity WHERE id = {$item['id']}");
            }
